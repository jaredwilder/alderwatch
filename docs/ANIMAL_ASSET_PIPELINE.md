...
8. Prefer semantic Idle/Walk/Run/Attack clip lookup and crossfade between them.
9. Run `npm test` and `npm run build`, then visually inspect the real game. Automated build success is not visual acceptance.

For the goat/sheep/deer/bear/bison/wolf set, `npm run assets:animals` downloads pinned licensed binaries. `predev` and `prebuild` invoke it automatically, so a fresh clone does not need Blender.
...
- Bear: uncommon solo predators. Bears hunt smaller prey and attack nearby/provoking players through authoritative predator combat.
- Wolf: Quaternius CC0 animated predators in packs capped at three. Wolves acquire bison, approach on deterministic flank points, and turn the whole pack onto a player who approaches/interferes with an active bison hunt or attacks a pack member/quarry.
