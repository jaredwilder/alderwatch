...
A second tree built in reverse order gives the same guarantee for westbound/reverse travel. Thus a rumor from source ward `s` to target ward `t` obtains an exact route certificate in logarithmic tree work after the hierarchy exists.
