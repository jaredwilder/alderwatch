...
test('world map intentionally releases capture without synthesizing Escape, then can recapture',t=>{const h=harness(t);h.input.active=true;h.input.keys.add('KeyW');h.input.secondary=true;h.win.fire('alderwatch:map-cursor',{detail:{open:true}});assert.equal(h.doc.pointerLockElement,null);assert.equal(h.input.captureCamera,false);assert.equal(h.input.keys.size,0);assert.equal(h.input.secondary,false);assert.equal(h.input.take('Escape'),false);h.win.fire('alderwatch:map-cursor',{detail:{open:false}});assert.equal(h.input.captureCamera,true);assert.equal(h.doc.pointerLockElement,h.canvas);});
