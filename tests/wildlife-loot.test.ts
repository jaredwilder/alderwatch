...
test('wolf packs can bring down bison through authoritative wildlife damage',()=>{
 ...
 const bite=wolfBite(a.state,wolf,bison);assert.equal(bite.damage,10);assert.equal(bite.killed,true);assert.equal(bison.killedBy,'wolf');const corpse=a.state.containers[corpseId(bison.id)];assert.ok(corpse);assert.equal(corpse.inventory.find(s=>s.item==='venison')?.count,8);
});
