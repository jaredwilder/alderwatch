...
export const MAX_MESSAGE = 400;   // one line of player speech
export const MAX_HISTORY = 8;     // turns kept for context, oldest dropped first
export const MAX_REPLY_TOKENS = 160;

// Shared world brief. Every persona is anchored to the same facts so two NPCs
// cannot contradict each other about where the player is standing.
...
/** @type {Record<string, {name:string, role:string, system:string}>} */
export const PERSONAS = {
  smith: {
    name: 'Rowan Ash',
    role: 'Blacksmith',
    system: `You are Rowan Ash, the blacksmith of Alderbrook. You are broad, blunt and tired.
...
${STYLE}`,
  ...
  cook: {
    name: 'Maerin Vale',
    role: 'Cook',
    system: `You are Maerin Vale, who keeps the roadside campfire at Alderbrook. You are warm,
...
${STYLE}`,
  ...
  warden: {
    name: 'Hallis Crow',
    role: 'Village warden',
    system: `You are Hallis Crow, the warden who watches Alderbrook's road. You are terse,
...
/** Collapse whitespace and hard-cap length. Player text is never trusted. */
...
/**
 * Validate a dialogue request body.
 * @returns {{ok:true, npcId:string, message:string, history:{role:string,content:string}[]}
...
export function validateRequest(body) {
  ...
  if (!Object.hasOwn(PERSONAS, npcId)) return { ok: false, status: 400, error: 'Unknown npcId' };

  const message = clean(body.message, MAX_MESSAGE);
  if (!message) return { ok: false, status: 400, error: 'Empty message' };

  // Only the two roles are accepted, and only the tail of the conversation is kept.
  const raw = Array.isArray(body.history) ? body.history : [];
  const history = raw
    .filter(t => t && (t.role === 'user' || t.role === 'assistant') && typeof t.content === 'string')
    .slice(-MAX_HISTORY)
    .map(t => ({ role: t.role, content: clean(t.content, MAX_MESSAGE) }))
    .filter(t => t.content);

  return { ok: true, npcId, message, history };
}

/** Build the exact message array sent upstream. The persona always wins position 0. */
export function buildMessages(npcId, message, history) {
  return [
    { role: 'system', content: PERSONAS[npcId].system },
    ...history,
...
/**
 * Fixed-window limiter, per key. Deliberately simple: this endpoint is small and
 * the process is single-node, so an in-memory window is the honest fit.
 */
