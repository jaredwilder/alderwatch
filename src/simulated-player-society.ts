import {SimulatedPlayerPopulation,PLAYER_BOTS} from './simulated-players';
...
const marker=Symbol.for('alderwatch.simulated-player-society.v1');
...
function runtimeById(pop:SimulatedPlayerPopulation,id:string){return (pop.runtimes as unknown as BotRuntimeLike[]).find(r=>r.definition.id===id);}
...
function setPresence(pop:SimulatedPlayerPopulation,r:BotRuntimeLike,p:Presence,online:boolean,time:number){p.online=online;p.sessions+=online?1:0;p.nextAt=time+(online?55+hash(r.definition.id+':on:'+p.sessions)%115:18+hash(r.definition.id+':off:'+p.sessions)%55);r.group.visible=online;append(pop,r.definition.name,r.player.id,online?choose(['yo','im back','sup','what did i miss','alright im on'],r.definition.id+':login:'+p.sessions):choose(['brb','im off for a bit','later','food. brb','gtg'],r.definition.id+':logout:'+p.sessions));}

function updatePresence(pop:SimulatedPlayerPopulation,s:SocietySave,time:number){const c=clocks.get(pop)!;if(time<c.nextPresence)return;c.nextPresence=time+6;ensurePresence(pop,s,time);for(const r of pop.runtimes as unknown as BotRuntimeLike[]){const p=s.presence[r.definition.id]!;if(time<p.nextAt)continue;const onlineCount=Object.values(s.presence).filter(x=>x.online).length;if(p.online&&onlineCount<=3){p.nextAt=time+25;continue;}setPresence(pop,r,p,!p.online,time);break;}const count=Object.values(s.presence).filter(x=>x.online).length+1;const label=document.querySelector('.player-realm-chat header strong');if(label)label.textContent=`${count} online`;persist(pop,s);}

function updateSocial(pop:SimulatedPlayerPopulation,s:SocietySave,time:number){const c=clocks.get(pop)!;if(time<c.nextSocial)return;c.nextSocial=time+17+hash(String(s.eventCounter))%24;const online=(pop.runtimes as unknown as BotRuntimeLike[]).filter(r=>s.presence[r.definition.id]?.online!==false);if(online.length<2)return;const a=online[s.eventCounter%online.length]!,b=online[(s.eventCounter*3+2)%online.length]!;s.eventCounter++;if(a===b)return;const k=relationKey(a.definition.id,b.definition.id),rel=s.relations[k]??={score:0,encounters:0,last:''};rel.encounters++;const chemistry=((hash(k+':'+rel.encounters)%200)-100)/100;rel.score=clamp(rel.score+chemistry*.34,-10,10);rel.last=chemistry>.35?'good run':chemistry<-.35?'argument':'chatted';s.relations[k]=rel;append(pop,a.definition.name,a.player.id,socialLine(a,b,rel.score,s.eventCounter));const response=replyLine(b,a,rel.score,s.eventCounter);setTimeout(()=>{if((s.presence[b.definition.id]?.online??true)&&document.body.contains(document.querySelector('.player-realm-chat')))append(pop,b.definition.name,b.player.id,response);},900+hash(k+':delay:'+rel.encounters)%2300);
 if(rel.score>3&&hash(k+':meet:'+rel.encounters)%3===0){b.memory.mode='come';b.memory.target=[a.player.position[0],a.player.position[2]];}persist(pop,s);}
...
function updateErrands(pop:SimulatedPlayerPopulation,s:SocietySave,time:number){const c=clocks.get(pop)!;for(const r of pop.runtimes as unknown as BotRuntimeLike[]){const e=s.errands[r.definition.id];if(!e)continue;const f=pop.state.forage[e.forageId];if(!f||f.harvested){delete s.errands[r.definition.id];r.memory.mode='wander';continue;}if(Math.hypot(f.position[0]-r.player.position[0],f.position[2]-r.player.position[2])<2.05){const out=authority(pop).dispatch({type:'forage',playerId:r.player.id,forageId:f.id});append(pop,r.definition.name,r.player.id,out.ok?choose(['got it','easy','mats secured','yoink'],r.definition.id+':got:'+f.id):`lol ${out.message.toLowerCase()}`);delete s.errands[r.definition.id];r.memory.mode='wander';persist(pop,s);}}
 if(time<c.nextErrand)return;c.nextErrand=time+22+hash('errand:'+s.eventCounter)%28;const available=(pop.runtimes as unknown as BotRuntimeLike[]).filter(r=>s.presence[r.definition.id]?.online!==false&&!s.errands[r.definition.id]&&r.memory.mode==='wander');if(!available.length)return;const r=available[hash(String(s.eventCounter)+':worker')%available.length]!,candidate=forageCandidates(pop.state,r.player)[0];if(candidate&&hash(candidate.id+':'+s.eventCounter)%4!==0){startErrand(pop,s,r,time,candidate);persist(pop,s);}}

function install(){const g=globalThis as any;if(g[marker])return;g[marker]=true;const proto=SimulatedPlayerPopulation.prototype as any,originalUpdate=proto.update,originalSend=proto.sendPlayerChat;proto.update=function(this:SimulatedPlayerPopulation,time:number){const out=originalUpdate.call(this,time);let s=saves.get(this);if(!s){s=safeLoad(this.state);saves.set(this,s);clocks.set(this,{nextSocial:time+12,nextPresence:time+5,nextErrand:time+18});ensurePresence(this,s,time);}updatePresence(this,s,time);updateSocial(this,s,time);updateErrands(this,s,time);return out;};proto.sendPlayerChat=function(this:SimulatedPlayerPopulation,text:string){let s=saves.get(this);if(!s){s=safeLoad(this.state);saves.set(this,s);clocks.set(this,{nextSocial:10,nextPresence:5,nextErrand:18});}const normalized=text.toLowerCase();for(const r of this.runtimes as unknown as BotRuntimeLike[]){const aliases=[r.definition.id,r.definition.name.toLowerCase(),r.definition.name.toLowerCase().replace(/[^a-z0-9_]/g,'')];if(aliases.some(a=>normalized.includes('@'+a)||normalized.startsWith(a+' '))){const d=chatAffinityDelta(text);r.memory.affinity=clamp(r.memory.affinity+d,-10,10);const h=human(this.state);if(h){const k=relationKey(r.definition.id,'human'),rel=s.relations[k]??={score:0,encounters:0,last:''};rel.score=clamp(rel.score+d,-10,10);rel.last=d>0?'player was kind':d<0?'player insulted them':'chat';rel.encounters++;s.relations[k]=rel;}persist(this,s);break;}}return originalSend.call(this,text);};}
