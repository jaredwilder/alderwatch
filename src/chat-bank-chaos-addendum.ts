...
/**
 * Recovered from the parallel `feat/chat-bank-chaos-expansion` branch after the
 * larger conversation-bank PR landed first. This is additive only: it never
 * replaces the current persona/topic banks or changes chat authority/routing.
 */
export const PLAYER_CHAT_CHAOS_ADDENDUM:Record<string,readonly string[]>={
 kestrel:[
...
export const NPC_CHAT_CHAOS_ADDENDUM:Record<string,readonly string[]>={
 mara:[
...
function mergeUnique(base:readonly string[],extra:readonly string[]){const seen=new Set(base);return [...base,...extra.filter(line=>!seen.has(line))];}
const installed=Symbol.for('alderwatch.chat-bank-chaos-addendum.v1');

export function installChatChaosAddendum(){
 const g=globalThis as Record<PropertyKey,unknown>;if(g[installed])return;g[installed]=true;
 for(const player of PLAYER_BOTS){const extra=PLAYER_CHAT_CHAOS_ADDENDUM[player.id]??[];(player as any).ambient=mergeUnique(player.ambient,extra);}
 for(const npc of NPC_ROSTER){const extra=NPC_CHAT_CHAOS_ADDENDUM[npc.id]??[];(npc as any).lines=mergeUnique(npc.lines,extra);}
}

if(typeof window!=='undefined')installChatChaosAddendum();
