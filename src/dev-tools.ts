...
function command(raw:string){const [op,...args]=raw.trim().split(/\s+/);if(!op)return 'Commands: status · tp x z · tp ironward · travel crossing|basin|mine|crownroad|greyhaven|wolfpine|march · heal · give ITEM [N] · spawnmark SPECIES [GROUP] · spawns · spawncopy · spawnpreview · spawnundo · spawnremove ID · spawnclear · reload';if(op==='status')return stateText();if(op==='reload'){reload();return 'reloading';}if(op==='heal'){heal();return 'healed';}if(op==='give'){give(args[0],Number(args[1]??1));return 'given';}if(op==='travel'){travel(args[0]??'crossing');return 'travelling';}if(op==='spawnmark'){beginWildlifeCapture(args[0]??'',args.slice(1).join('-'));return 'capturing';}if(op==='spawns')return wildlifeSummary();if(op==='spawncopy')return copyWildlifeExport();if(op==='spawnpreview')return refreshWildlifePreviews();if(op==='spawnundo')return undoWildlifeMark();if(op==='spawnremove')return removeWildlifeMark(args[0]??'');if(op==='spawnclear')return clearWildlifeMarks();if(op==='spawnclearpreview')return clearWildlifePreviews();if(op==='tp'){if(args[0]==='ironward'){teleport(IRONWARD_CROSSING_GATE.x,IRONWARD_CROSSING_GATE.z);return 'teleporting';}const x=Number(args[0]),z=Number(args[1]);if(!Number.isFinite(x)||!Number.isFinite(z))throw new Error('Usage: tp X Z');teleport(x,z);return 'teleporting';}throw new Error(`Unknown command: ${op}`);}
