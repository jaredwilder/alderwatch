...
export class StreamedAreaSurfaceSet{
 readonly ground:T.MeshStandardMaterial;readonly road:T.MeshStandardMaterial;readonly yard:T.MeshStandardMaterial;private owned:T.Texture[]=[];private geometries=new Map<string,T.PlaneGeometry>();
 constructor(assets:Assets,palette:StreamedAreaSurfacePalette){
  ...
  this.yard=new T.MeshStandardMaterial({map:road,color:palette.yard??palette.road,roughness:1,metalness:0});this.yard.name='AW_StreamedYardShared';
 }
 ...
 dispose(){this.ground.dispose();this.road.dispose();this.yard.dispose();for(const texture of this.owned)texture.dispose();for(const geometry of this.geometries.values())geometry.dispose();this.owned=[];this.geometries.clear();}
}
