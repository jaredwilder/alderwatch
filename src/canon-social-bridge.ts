...
function promoteMessage(pop:SimulatedPlayerPopulation,m:Message,keyPrefix:string){if(!canonWorthySocialText(m.text))return;const source=sourceFor(m.speakerId),actorId=m.speakerId,actorName=m.name||m.speakerId,ward=actorWard(pop,m.speakerId),key=`${keyPrefix}:${m.speakerId}:${m.at}:${hash(m.text)}`;promoteCanonicalEvent(pop.state,{source,actorId,actorName,ward,channel:socialChannelForText(m.text),externalKey:key,summary:`${actorName}: “${m.text.slice(0,180)}” — the statement became consequential enough to enter realm memory.`});}

function scanNpcNetwork(pop:SimulatedPlayerPopulation,seen:Set<string>){for(const line of Array.from(document.querySelectorAll<HTMLElement>('.realm-channel-npc .npc-network-line')).slice(-24)){const name=line.querySelector('b')?.textContent?.trim()??'Unknown',text=line.querySelector('span')?.textContent?.trim()??'';if(!text)continue;const sig=`${name}|${text}`;if(seen.has(sig))continue;seen.add(sig);if(seen.size>160)seen.delete(seen.values().next().value!);if(!canonWorthySocialText(text))continue;const humanLine=line.classList.contains('human'),p=human(pop),actorId=humanLine?(p?.id??'player'):`npc:${name.toLowerCase().replace(/[^a-z0-9]+/g,'-')}`,source:CanonSource=humanLine?'player':'npc';promoteCanonicalEvent(pop.state,{source,actorId,actorName:humanLine?(p?.name??name):name,ward:areaWard((p as AreaPlayer|undefined)?.areaId),channel:socialChannelForText(text),externalKey:`npc-chat:${hash(sig)}`,summary:`${humanLine?(p?.name??name):name} put this into the NPC relay: “${text.slice(0,180)}”. It now has an exact causal source.`});}}
