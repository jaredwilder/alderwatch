...
export async function talkToNpc(npcId:string,message:string,world:WorldState,player:PlayerState,history:readonly DialogueTurn[],region='The Far March',systemContext=''):Promise<NpcChatResult>{
 ...
 try{
  const systemPrompt=openRouterSystemPrompt(npcId,ctx)+(systemContext.trim()?` ${systemContext.trim().slice(0,1100)}`:'');
  const response=await fetch('/api/npc-chat',{method:'POST',headers:{'Content-Type':'application/json'},signal:controller.signal,body:JSON.stringify({npcId,npcName:persona.name,role:persona.role,persona:systemPrompt,message:message.slice(0,400),history:compactHistory(history),context:ctx})});
