...
export class Building {
 ...
 constructor(public world:T.Group,public assets:Assets,public physics:RAPIER.World,public authority:LocalAuthority){this.sync();}
 model(kind:BuildKind){const group=new T.Group();group.add(this.assets.prop(kind));if(kind==='doorway'){const door=this.assets.prop('door');door.name='hinged-door';door.position.set(-.54,0,-.10);group.add(door);for(let i=0;i<2;i++){const step=this.assets.prop('foundation');step.name='Threshold step '+i;step.scale.set(.38,.34,.16);step.position.set(0,-.44+i*.15,.78-i*.38);group.add(step);}}if(kind==='roof')for(const side of [-1,1]){const gable=this.assets.prop('gable');gable.name='gable-'+side;gable.position.z=side*1.5;gable.rotation.y=side<0?Math.PI:0;group.add(gable);}return group;}
 ...
 candidate(p:PlayerState,point:Vec3):Placement{
  ...
  if(this.kind==='foundation'){
   pos=[Math.round(point[0]/3)*3,0,Math.round(point[2]/3)*3];pos[1]=height(pos[0],pos[2]);
  ...
  }else{pos[0]=Math.round(pos[0]*2)/2;pos[2]=Math.round(pos[2]*2)/2;if(near&&Math.abs(pos[0]-near.position[0])<1.4&&Math.abs(pos[2]-near.position[2])<1.4){supportId=near.id;pos[1]=near.position[1]+.47;}else pos[1]=height(pos[0],pos[2]);}
 ...
 update(camera:T.Camera,pointer:T.Vector2,p:PlayerState,dt=1/60){
  ...
  for(let t=.5;t<32;t+=.12){const v=this.ray.ray.at(t,new T.Vector3());let ground=height(v.x,v.z);for(const f of floors)if(Math.abs(v.x-f.position[0])<1.5&&Math.abs(v.z-f.position[2])<1.5)ground=Math.max(ground,f.position[1]+.47);if(v.y<=ground){point=[v.x,ground,v.z];break;}}
