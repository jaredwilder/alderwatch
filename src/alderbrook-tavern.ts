...
import {TAVERN_DRINKS,MAX_INTOXICATION,addIntoxication,intoxicationLabel,resolveAlderbones,sober,tavernCameraSway,tavernMoveScale,type TavernDrinkId} from './tavern-rules';
import './alderbrook-tavern.css';

export const TIPSY_ALDER_NAME='The Tipsy Alder';
const EXTERIOR_X=-9.3,EXTERIOR_Z=-29.4;
// Dedicated interior layer: same Alderbrook X/Z for stable realm semantics, but far above
// every streamed landscape/boss and beyond the outdoor camera far plane.
const INTERIOR=new T.Vector3(EXTERIOR_X,3000,EXTERIOR_Z);
const SPAWN_LOCAL=new T.Vector3(0,.12,3.65);
const EXIT_LOCAL=new T.Vector3(0,.12,6.3);
...
const PATRONS=[
 ...
 {id:'jorren',name:'Jorren Pike',at:[-.4,0,3.15] as const,yaw:3.0,hair:'#6a513c',cloth:'#625868',lines:["Jorren raises a cup. ‘To Alderbrook: where every roof leaks in a different place.’","‘Crow’s Regret is not technically poison. Brinna has a parchment saying so.’","‘I came in for one. Then Pell explained arithmetic.’"]},
] as const;
...
function canvasSign(){
 ...
 c.fillStyle='#24170f';c.fillRect(0,0,768,384);c.strokeStyle='#b99a58';c.lineWidth=16;c.strokeRect(22,22,724,340);c.strokeStyle='#6f5530';c.lineWidth=5;c.strokeRect(43,43,682,298);
 c.fillStyle='#dbc47e';c.textAlign='center';c.font='700 60px Georgia';c.fillText('THE TIPSY',410,142);c.font='700 82px Georgia';c.fillText('ALDER',410,229);
 c.font='italic 27px Georgia';c.fillStyle='#c6aa67';c.fillText('ALE · BONES · BAD COUNSEL',410,300);
 c.strokeStyle='#d3b86f';c.lineWidth=11;c.beginPath();c.arc(105,190,39,.4,Math.PI*1.7);c.stroke();c.fillStyle='#d3b86f';c.fillRect(79,153,52,72);c.strokeRect(126,169,29,38);
...
export class AlderbrookTavern{
 inside=false;intoxication=0;smoke=0;
 private actors:TavernActor[]=[];private warmLights:T.PointLight[]=[];private bonesWins=0;private bonesLosses=0;private pipePulls=0;
 private interiorRoot=new T.Group();
 ...
 private buildStreetIdentity(){
  ...
  const board=new T.Mesh(new RoundedBoxGeometry(2.12,.93,.1,3,.055),new T.MeshStandardMaterial({map:canvasSign(),roughness:.86,metalness:.02}));board.position.set(.15,2.72,.58);board.rotation.y=Math.PI;board.castShadow=true;root.add(board);
  const iron=new T.MeshStandardMaterial({color:'#292622',roughness:.55,metalness:.58});
  const arm=new T.Mesh(new RoundedBoxGeometry(.08,.08,.72,2,.018),iron);arm.position.set(.15,3.18,.24);root.add(arm);
  const hanger=new T.Mesh(new RoundedBoxGeometry(.055,.5,.055,2,.015),iron);hanger.position.set(.15,2.92,.56);root.add(hanger);
  this.authored(root,'lantern',-1.22,1.72,.34,Math.PI,.58);const light=new T.PointLight('#ffae62',2.4,6.5,2);light.position.set(-1.22,2.02,.42);root.add(light);
 }
 ...
 private plasterMaterial(){return new T.MeshStandardMaterial({color:'#b9aa8c',roughness:.98,metalness:0});}
 ...
 private table(parent:T.Group,x:number,z:number,yaw=0,dice=false){const wood=this.timberMaterial(),g=new T.Group();g.position.set(x,0,z);g.rotation.y=yaw;parent.add(g);this.box(g,[2.6,.16,1.35],[0,.91,0],wood,.07);for(const sx of [-1,1])for(const sz of [-.45,.45])this.box(g,[.17,.86,.17],[sx,.43,sz],wood,.045);for(const side of [-1,1]){this.box(g,[2.25,.14,.38],[0,.53,side*1.02],wood,.05);for(const lx of [-.82,.82])this.box(g,[.15,.48,.15],[lx,.24,side*1.02],wood,.035);}this.collider(x,.55,z,1.25,.55,.7);if(dice){const felt=new T.MeshStandardMaterial({color:'#354c39',roughness:1});this.box(g,[1.4,.025,.72],[0,1.005,0],felt,.02);for(const dx of [-.27,.27]){const die=this.box(g,[.16,.16,.16],[dx,1.12,.02],new T.MeshStandardMaterial({color:'#d7c79e',roughness:.78}),.035);die.rotation.set(.3+dx,.2-dx,.5);}}}
 private counter(parent:T.Group){const wood=this.darkTimberMaterial();this.box(parent,[8.4,.18,1.05],[0,1.08,-5.25],wood,.08);this.box(parent,[8.15,1.0,.18],[0,.53,-5.62],wood,.045);for(const x of [-3.7,-2.2,-.7,.8,2.3,3.8])this.box(parent,[.14,1.0,.8],[x,.53,-5.28],wood,.035);this.collider(0,.65,-5.25,4.25,.65,.54);}
 private actor(id:string,name:string,local:[number,number,number],yaw:number,hair:string,cloth:string,lines:string[]){
  const root=new T.Group(),model=this.assets.human();root.position.set(...local);root.rotation.y=yaw;root.add(model);this.interiorRoot.add(root);model.traverse(o=>{if(o.name.includes('Hood'))o.visible=false;if(o.name.includes('Hair_Simple'))o.visible=true;if(!(o instanceof T.Mesh))return;const mats=Array.isArray(o.material)?o.material:[o.material];const dressed=mats.map(old=>{const m=(old as T.MeshStandardMaterial).clone();weatheredCloth(m,this.assets.textures,o.name);if(m.name.includes('Hair'))m.color.set(hair);if(m.name.includes('Ranger'))m.color.set(cloth);return m;});o.material=Array.isArray(o.material)?dressed:dressed[0];});const mixer=new T.AnimationMixer(model),idle=this.assets.survivor.animations.find(a=>a.name==='idle');if(!idle)throw new Error('Tavern actor requires idle animation');mixer.clipAction(idle).play().time=(id.length*.71)%Math.max(.1,idle.duration);const actor={id,name,root,mixer,baseYaw:yaw,local:new T.Vector3(...local),lines,line:0};this.actors.push(actor);return actor;
 }
 private buildInterior(){
  const root=this.interiorRoot;root.name='The Tipsy Alder · isolated interior capsule';root.position.copy(INTERIOR);root.visible=false;this.scene.add(root);
  const timber=this.darkTimberMaterial(),plaster=this.plasterMaterial(),floorMat=this.timberMaterial();
  // Structural shell is intentionally primitive and watertight. Authored kit is decoration only.
  const floor=this.box(root,[13.8,.2,14.4],[0,-.1,0],floorMat,.025);floor.receiveShadow=true;
  this.box(root,[13.8,.22,14.4],[0,4.05,0],timber,.025);
  this.box(root,[13.8,4.0,.24],[0,1.95,-7.08],plaster,.025);this.box(root,[13.8,4.0,.24],[0,1.95,7.08],plaster,.025);
  this.box(root,[.24,4.0,14.0],[-6.78,1.95,0],plaster,.025);this.box(root,[.24,4.0,14.0],[6.78,1.95,0],plaster,.025);
  this.collider(0,-.13,0,6.9,.13,7.2);this.collider(0,1.95,-7.08,6.9,2,.14);this.collider(0,1.95,7.08,6.9,2,.14);this.collider(-6.78,1.95,0,.14,2,7);this.collider(6.78,1.95,0,.14,2,7);
  // Beams give the plain shell adult medieval structure without depending on modular wall orientation.
  for(const x of [-6.55,-3.3,0,3.3,6.55]){this.box(root,[.18,3.85,.18],[x,1.92,-6.91],timber,.035);this.box(root,[.18,3.85,.18],[x,1.92,6.91],timber,.035);}
  for(const z of [-6.8,-3.4,0,3.4,6.8]){this.box(root,[.18,3.85,.18],[-6.61,1.92,z],timber,.035);this.box(root,[.18,3.85,.18],[6.61,1.92,z],timber,.035);}
  for(const z of [-5.6,-2.8,0,2.8,5.6])this.box(root,[13.25,.16,.2],[0,3.72,z],timber,.035);
  // Interior exit reads as a real oak door but remains a sealed instance boundary.
  const door=this.box(root,[1.55,2.7,.12],[0,1.35,6.89],this.darkTimberMaterial(),.055);door.castShadow=true;
  for(const y of [.36,1.08,1.8,2.52])this.box(root,[1.38,.06,.05],[0,y,6.80],timber,.015);
  const latch=new T.Mesh(new T.SphereGeometry(.075,10,8),new T.MeshStandardMaterial({color:'#40342a',roughness:.5,metalness:.6}));latch.position.set(.5,1.25,6.72);root.add(latch);
  this.counter(root);this.table(root,-3.7,.9,.08,true);this.table(root,3.45,1.75,-.18);this.table(root,-.35,3.0,.04);
  this.authored(root,'barrel',-4.9,0,-6.35,.2,.8);this.authored(root,'barrel',4.9,0,-6.3,-.1,.82);this.authored(root,'barrel',5.55,0,-5.65,.15,.72);this.authored(root,'crate',-5.45,0,-5.7,.1,.7);this.authored(root,'cauldron',5.05,0,-2.8,.1,.85);this.authored(root,'campfire_burning_q',5.05,0,-3.0,0,.72);this.collider(5.05,.38,-3.0,.65,.38,.65);
  for(const [x,z] of [[-5.3,-1.6],[5.25,3.7],[-5.1,5.5]] as const){this.authored(root,'lantern',x,1.65,z,0,.62);const l=new T.PointLight('#ffac63',3.6,10,2);l.position.set(x,2.15,z);root.add(l);this.warmLights.push(l);}
  const centre=new T.PointLight('#ffd08b',4.1,13,2);centre.position.set(0,2.8,.8);root.add(centre);this.warmLights.push(centre);const hearth=new T.PointLight('#ff7a32',5.4,10,2);hearth.position.set(5.05,1.15,-3);root.add(hearth);this.warmLights.push(hearth);
  const mugGeo=new T.CylinderGeometry(.09,.075,.22,10),mugMat=new T.MeshStandardMaterial({color:'#a98b5f',roughness:.9}),mugs=new T.InstancedMesh(mugGeo,mugMat,10),dummy=new T.Object3D();const mugSpots=[[-3.3,1.08,.7],[-4.0,1.08,1.0],[3.1,1.08,1.55],[3.8,1.08,1.9],[-.5,1.08,2.8],[.2,1.08,3.2],[-2.2,1.22,-5.15],[-1.3,1.22,-5.15],[1.2,1.22,-5.15],[2.1,1.22,-5.15]];mugSpots.forEach((p,i)=>{dummy.position.set(p[0],p[1],p[2]);dummy.rotation.y=i*.73;dummy.updateMatrix();mugs.setMatrixAt(i,dummy.matrix);});mugs.castShadow=true;root.add(mugs);
  this.actor('brinna','Brinna Keggs',[0,0,-6.35],0,'#5b3824','#795843',["‘Drink first. Confess later. I charge less for the first part.’"]);for(const p of PATRONS)this.actor(p.id,p.name,[p.at[0],p.at[1],p.at[2]],p.yaw,p.hair,p.cloth,[...p.lines]);
  this.authored(root,'crate',4.65,0,-1.6,.25,.72);this.authored(root,'lantern',5.35,1.25,-1.55,0,.52);
 }
 ...
 read(){return{name:TIPSY_ALDER_NAME,inside:this.inside,intoxication:this.intoxication,smoke:this.smoke,bones:{wins:this.bonesWins,losses:this.bonesLosses},exteriorDoor:[...this.exteriorDoor],interiorSpawn:[...this.interiorSpawn],interiorVisible:this.interiorRoot.visible,interiorLayerY:INTERIOR.y};}
}
