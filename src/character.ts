...
export class Character {
 ...
 getTick=()=>0;onActionRequest=(action:'attack'|'heavy'|'dodge')=>beginAction(this.state,this.getTick(),action).ok;onWindupAim=(_dt:number)=>{};private poseKey='';private swingLocomotion=false;private strideAction?:T.AnimationAction;private strideActive=false;knockback=new T.Vector3();
 ...
 preStep(dt:number,input:Pick<Input,'keys'|'take'|'secondary'> & Partial<Pick<Input,'primary'>>,yaw:number,enabled:boolean){
  ...
  const walkingSwing=this.swingLocomotion&&['attack','heavy'].includes(combatState(p).kind)&&combatState(p).until>tick;if(this.strideAction){if(walkingSwing!==this.strideActive){walkingSwing?this.strideAction.fadeIn(.12):this.strideAction.fadeOut(.12);this.strideActive=walkingSwing;}this.strideAction.timeScale=Math.max(.1,target.length()/1.5);}
