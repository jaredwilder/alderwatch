...
export class Character {
 ...
 private strideAmplitude(gait:string){return gait==='sprint'?.62:gait==='run'?.70:.78;}
 private prepareStride(name:string,phase?:number){const gait=name==='guard_walk'?'walk':name,next=this.strideActions.get(gait);if(!next)return;if(phase!==undefined&&this.strideAction)this.setPhase(this.strideAction,phase);if(this.activeStrideAction!==next){const previous=this.activeStrideAction,handoff=phase??(previous?this.phase(previous):this.strideAction?this.phase(this.strideAction):0);this.setPhase(next,handoff);if(this.strideActive){previous?.fadeOut(.055);next.enabled=true;next.weight=this.strideAmplitude(gait);next.fadeIn(.055);}this.activeStrideAction=next;}else if(phase!==undefined)this.setPhase(next,phase);}
 private updateStride(speed:number,active:boolean){const gait=speed>4.7?'sprint':speed>2.35?'run':'walk';this.prepareStride(gait);const action=this.activeStrideAction;if(!action)return;const amplitude=this.strideAmplitude(gait);if(active!==this.strideActive){if(active){action.enabled=true;action.weight=amplitude;action.fadeIn(.055);}else action.fadeOut(.08);this.strideActive=active;}else if(active){action.enabled=true;action.weight=amplitude;}const nominal=gait==='sprint'?6.4:gait==='run'?3.6:1.5;
  // Combat footwork covers more world distance per visual step: slower cadence + reduced lower-body blend
  // keeps the feet alive without the high-knee marching that a full-speed locomotion loop produced.
  action.timeScale=T.MathUtils.clamp(speed/nominal*.80,.58,1.04);if(this.strideAction&&this.strideAction!==action){this.setPhase(this.strideAction,this.phase(action));this.strideAction.timeScale=action.timeScale;}}
 ...
 preStep(dt:number,input:Pick<Input,'keys'|'take'|'secondary'> & Partial<Pick<Input,'primary'>>,yaw:number,enabled:boolean){
  ...
  const liveCombat=combatState(p),activeSwing=liveCombat.weapon!=='bow'&&['attack','heavy'].includes(liveCombat.kind)&&liveCombat.until>tick,strideSpeed=Math.max(target.length(),this.velocity.length(),(activeSwing&&!this.swingLocomotion)?0.9:0);this.updateStride(strideSpeed,activeSwing);
