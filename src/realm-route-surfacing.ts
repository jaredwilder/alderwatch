...
async function atlasData(areaId:string,world:WorldState):Promise<{name:string;bounds:AtlasBounds;landmarks:AtlasLandmark[];footer:string}|undefined>{
 if(areaId===CROWNROAD_VALE){
  const {CROWNROAD_CELL,CROWNROAD_HALF,CROWNROAD_POIS}=await import('./crownroad-world');
  const kind=(value:string):AtlasLandmark['kind']=>value==='city'||value==='village'?'settlement':value==='fort'||value==='crossing'?'gate':value==='ruin'?'danger':'site';
  return{name:'Crownroad Vale · Greyhaven',bounds:{minX:-CROWNROAD_HALF,maxX:CROWNROAD_HALF,minZ:-CROWNROAD_HALF,maxZ:CROWNROAD_HALF},landmarks:CROWNROAD_POIS.map(p=>({label:p.name,position:[p.cell.x*CROWNROAD_CELL,0,p.cell.z*CROWNROAD_CELL],kind:kind(p.kind)})),footer:"Greyhaven is at the centre of the King's Road · west returns to Gatewatch"};
 }
 if(areaId===DEEP_IRON_MINE){
  const {DEEP_IRON_ROOMS,ensureDeepIron,nearestDeepIronRoom}=await import('./deep-iron-dungeon'),state=ensureDeepIron(world),player=currentPlayer(world),here=player&&nearestDeepIronRoom(player.position),known=new Set([...state.discovered,'entrance',...(here?[here.id]:[])]);
  return{name:'Deep Iron Mine',bounds:{minX:-52,maxX:52,minZ:-8,maxZ:106},landmarks:DEEP_IRON_ROOMS.filter(r=>known.has(r.id)).map(r=>({label:r.name,position:r.position,kind:r.feature==='lower-gate'?'danger':r.id==='entrance'?'gate':'site'})),footer:'Only discovered workings are charted · the mine entrance returns to Gatewatch'};
 }
}
async function openFallbackAtlas(areaId:string){
 if(atlasOpening||fallbackAtlas?.isConnected)return;atlasOpening=true;
 try{
  const world=loadWorld(),player=world&&currentPlayer(world),data=world&&await atlasData(areaId,world);if(!world||!player||!data)return;
  const overlay=document.createElement('section');overlay.className='area-map-overlay';overlay.setAttribute('role','dialog');overlay.setAttribute('aria-label',`${data.name} map`);overlay.innerHTML='<div class="area-map-card"><header><div><small>ALDERWATCH REALM ATLAS</small><h2></h2></div><button type="button">M / ESC · CLOSE</button></header><div class="area-map-field"><div class="area-map-grid"></div></div><footer><span>YOU</span><span></span></footer></div>';
  overlay.querySelector('h2')!.textContent=data.name.toUpperCase();overlay.querySelector<HTMLElement>('footer span:last-child')!.textContent=data.footer;overlay.querySelector<HTMLButtonElement>('header button')!.onclick=closeFallbackAtlas;
  const field=overlay.querySelector<HTMLElement>('.area-map-field')!,p=mapPoint(player.position,data.bounds),you=document.createElement('i');you.className='area-map-you';you.style.left=p.x+'%';you.style.top=p.y+'%';you.title='You';field.append(you);
  for(const landmark of data.landmarks){const q=mapPoint(landmark.position,data.bounds),pin=document.createElement('span');pin.className='area-map-pin '+landmark.kind;pin.style.left=q.x+'%';pin.style.top=q.y+'%';pin.innerHTML='<b></b><em></em>';pin.querySelector('b')!.textContent=landmark.kind==='gate'?'◆':landmark.kind==='settlement'?'⌂':landmark.kind==='danger'?'▲':'●';pin.querySelector('em')!.textContent=landmark.label;field.append(pin);}
  document.body.append(overlay);fallbackAtlas=overlay;cursor(true);
 }finally{atlasOpening=false;}
}
function installFallbackAtlas(areaId:string){
 if(areaId!==DEEP_IRON_MINE&&areaId!==CROWNROAD_VALE)return;
