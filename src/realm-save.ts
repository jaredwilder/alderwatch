...
export interface PersistentAreaRecord {
  id: AreaId;
...
declare module './state' {
  interface PlayerState {
    areaId?: AreaId;
    areaPositions?: Record<AreaId,SavedAreaLocation>;
  }
  interface WorldState {
    realmAreas?: Record<AreaId,PersistentAreaRecord>;
  }
}
...
export function currentPlayer(world:WorldState):PlayerState|undefined {
  return Object.values(world.players)[0];
}

export function migrateRealmSave(world:WorldState):WorldState {
  world.realmAreas??={};
  for(const player of Object.values(world.players)){
    player.areaId??=FAR_MARCH;
    player.areaPositions??={};
  }
  return world;
}

export function playerArea(player:PlayerState):AreaId {
  return player.areaId??FAR_MARCH;
}

export function rememberCurrentArea(player:PlayerState):void {
  const area=playerArea(player);
  player.areaPositions??={};
  player.areaPositions[area]={position:[...player.position],yaw:player.yaw};
}

export function enterSavedArea(world:WorldState,target:AreaId):PlayerState {
  migrateRealmSave(world);
  const player=currentPlayer(world);
  if(!player)throw new Error('Cannot change realm area without a survivor');
  const outgoing=playerArea(player);
  rememberCurrentArea(player);
  const fallback=AREA_ENTRY[target]??AREA_ENTRY[FAR_MARCH];
  const destination=player.areaPositions?.[target]??fallback;
  player.areaId=target;
  player.position=[...destination.position];
  player.yaw=destination.yaw;
...
export function requestAreaTravel(target:AreaId):void {
  sessionStorage.setItem('alderwatch.pendingArea',target);
}

export function consumePendingArea():AreaId|undefined {
  const target=sessionStorage.getItem('alderwatch.pendingArea') as AreaId|null;
  if(target)sessionStorage.removeItem('alderwatch.pendingArea');
