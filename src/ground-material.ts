...
function woodlandGround(textures:Record<string,T.Texture>){
 ...
 material.onBeforeCompile=s=>{
  ...
  s.fragmentShader=s.fragmentShader.replace('#include <map_fragment>',`
   ...
   float awFarStat=smoothstep(92.0,158.0,awViewDistance);
   ...
   // Once a texel pattern projects beneath useful visual bandwidth, stop asking
   // a repeating image to describe a hill. A bounded statistical surface takes
   // over, driven by world-space ecology frequencies that cannot tile.
   vec3 fieldStat=mix(vec3(.34,.43,.235),vec3(.49,.57,.315),clamp(.24+.62*macro+.14*meso,0.0,1.0));
   vec3 litterStat=mix(vec3(.245,.235,.175),vec3(.355,.34,.225),clamp(.22+.54*macro+.24*(1.0-meso),0.0,1.0));
   vec3 field=fieldStat,litter=litterStat;
   if(awFarStat<.995){
    field=texture2D(awField,fieldUvA).rgb;litter=texture2D(awLitter,litterUvA).rgb;
    ...
    field=mix(field,fieldStat,awFarStat);litter=mix(litter,litterStat,awFarStat);
   }
  ...
  s.fragmentShader=s.fragmentShader.replace('#include <roughnessmap_fragment>',`#include <roughnessmap_fragment>
   if(awGroundDetailQuality>.52&&awViewDistance<118.0){
  ...
  `);
 };material.customProgramCacheKey=()=> 'aw-observer-detail-ground-v5-statistical-horizon';return material;
}
