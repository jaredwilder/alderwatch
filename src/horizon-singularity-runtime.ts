...
import {HORIZON_CANOPY_FIELDS,enteringHorizonCells,horizonCanopyQuality,horizonFieldOrigin,horizonHash,horizonRidgeWoodlandBias,horizonTerrainSupport,type CanopyBandSpec,type HorizonGridOrigin} from './horizon-singularity';
...
function tetra(g:G,cx:number,cy:number,cz:number,rx:number,ry:number,rz:number,color:T.Color,phase=0){
 const top:P=[cx,cy+ry,cz],base:P[]=[];
 for(let i=0;i<3;i++){const a=phase+i*Math.PI*2/3;base.push([cx+Math.cos(a)*rx,cy-ry*.48,cz+Math.sin(a)*rz]);}
 tri(g,top,base[0],base[1],color);tri(g,top,base[1],base[2],color);tri(g,top,base[2],base[0],color);tri(g,base[0],base[2],base[1],color);
}
...
export function createHorizonCanopyGeometry(spec:CanopyBandSpec){
 ...
 if(spec.id==='crown'){
  // Eight trunk triangles plus six overlapping four-triangle leaf lobes. The
  // irregular lobe cloud reads as a tree crown at distance instead of three
  // obvious octahedral gemstones while keeping the exact 32-triangle ceiling.
  const r=.17,y=2.95;for(let i=0;i<4;i++){const a=i*Math.PI/2,b=(i+1)*Math.PI/2;quad(g,[Math.cos(a)*r,0,Math.sin(a)*r],[Math.cos(b)*r,0,Math.sin(b)*r],[Math.cos(b)*r,y,Math.sin(b)*r],[Math.cos(a)*r,y,Math.sin(a)*r],i%2?bark:barkLight);}
  tetra(g,-.82,4.35,.15,1.38,1.62,1.24,leafDark,.15);
  tetra(g,.62,4.42,-.48,1.42,1.70,1.30,leaf,.82);
  tetra(g,.08,5.30,.28,1.55,1.55,1.42,leafLight,1.55);
  tetra(g,-1.28,5.02,-.34,1.05,1.18,1.02,leaf,.42);
  tetra(g,1.27,5.18,.30,1.08,1.25,1.04,leafDark,1.18);
  tetra(g,.06,6.02,-.18,.98,1.10,.92,leafLight,2.10);
 }else{
  ...
  quad(g,[-2.9,.32,-.48],[2.9,.32,.48],[2.25,3.18,.34],[-2.25,3.18,-.34],leaf);
  quad(g,[.46,.28,-2.55],[-.46,.28,2.55],[-.34,3.08,2.0],[.34,3.08,-2.0],leafLight);
 }
...
function canopyMaterial(spec:CanopyBandSpec){
 ...
 material.onBeforeCompile=shader=>{
  ...
  shader.vertexShader='varying vec2 awHorizonAnchor;varying vec3 awHorizonWorld;\n'+shader.vertexShader;shader.vertexShader=shader.vertexShader.replace('void main() {',`void main() {
   vec4 awHLocal=vec4(position,1.0),awHAnchorLocal=vec4(0.0,0.0,0.0,1.0);
   #ifdef USE_INSTANCING
    awHLocal=instanceMatrix*awHLocal;awHAnchorLocal=instanceMatrix*awHAnchorLocal;
   #endif
   awHorizonWorld=(modelMatrix*awHLocal).xyz;
   awHorizonAnchor=(modelMatrix*awHAnchorLocal).xz;
  `);
  const p=spec.metricPower.toFixed(2),ip=(1/spec.metricPower).toFixed(7),phase=((spec.seed%991)/991).toFixed(7);
  shader.fragmentShader='uniform vec2 awHorizonCenter;uniform float awHorizonQuality;varying vec2 awHorizonAnchor;varying vec3 awHorizonWorld;\n'+shader.fragmentShader;
  shader.fragmentShader=shader.fragmentShader.replace('#include <color_fragment>',`#include <color_fragment>
   ...
   // Fade coverage, never object scale and never camera pitch. Quantizing in
   // world space makes the transition spatially stable while the camera moves.
   vec3 awHQ=floor(awHorizonWorld*2.15+vec3(${(phase*19).toFixed(4)},${(phase*37).toFixed(4)},${(phase*53).toFixed(4)}));
   float awHDither=fract(sin(dot(awHQ,vec3(12.9898,78.233,37.719)))*43758.5453123);
   float awHCoverage=awHVisible*awHVisible*(3.0-2.0*awHVisible);
   if(awHDither>awHCoverage)discard;
 ...
 material.customProgramCacheKey=()=>`aw-horizon-canopy-${spec.id}-v4-stable-coverage`;material.needsUpdate=true;return material;
}
...
class ToroidalCanopyBand{
 mesh:T.InstancedMesh;origin?:HorizonGridOrigin;dummy=new T.Object3D();color=new T.Color();normal=new T.Vector3();slope=new T.Quaternion();writes=0;activeCount=0;
 ...
 private write(gx:number,gz:number,slot:number){
  const {landscape,spec}=this,jx=(horizonHash(gx,gz,spec.seed+11)-.5)*spec.cell*.82,jz=(horizonHash(gx,gz,spec.seed+23)-.5)*spec.cell*.82,x=(gx+.5)*spec.cell+jx,z=(gz+.5)*spec.cell+jz,y=height(x,z),road=roadDistance(x,z),woods=forestDensity(x,z),edge=forestEdge(x,z),eco=ecologyState(x,z,roadDisturbance(road)),support=horizonTerrainSupport(x,z),ridgeBias=horizonRidgeWoodlandBias(x,z),effectiveWoods=Math.min(1,woods+ridgeBias);
  const chance=Math.min(.975,Math.max(0,(effectiveWoods*.84+edge*.24+eco.understory*.12-.08)*spec.density*support)),blocked=support<=.01||!Number.isFinite(y)||y<-2.4||road<8.2||effectiveWoods<.12||blockedByBuild(landscape,x,z)||horizonHash(gx,gz,spec.seed+71)>chance;
  if(blocked){this.active[slot]=0;return;}
  const phenotype=treePhenotype(x,z),random=.78+horizonHash(gx,gz,spec.seed+101)*.48,scale=spec.scale*random,step=spec.id==='mass'?2.6:1.5;
  this.dummy.position.set(x,y-.22,z);this.dummy.rotation.set(phenotype.leanX*.22,horizonHash(gx,gz,spec.seed+127)*Math.PI*2+phenotype.yawJitter,phenotype.leanZ*.22);
  this.normal.set(height(x-step,z)-height(x+step,z),step*2.15,height(x,z-step)-height(x,z+step)).normalize();this.slope.setFromUnitVectors(UP,this.normal);this.dummy.quaternion.premultiply(this.slope);
  this.dummy.scale.set(scale*phenotype.widthX,scale*phenotype.height,scale*phenotype.widthZ);this.dummy.updateMatrix();this.matrices[slot].copy(this.dummy.matrix);this.active[slot]=1;
  this.color.set('#557743');this.color.offsetHSL((horizonHash(gx,gz,spec.seed+149)-.5)*.024,(eco.moisture-.5)*.08,(eco.fertility-.5)*.06-(effectiveWoods-.5)*.025);this.colors[slot].copy(this.color);
 }
