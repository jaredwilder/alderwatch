...
function leafDepth(material:T.MeshStandardMaterial){
 ...
 material.onBeforeCompile=(shader,renderer)=>{
  ...
  `);
  shader.fragmentShader='varying vec2 awTreeAnchor;varying vec3 awLeafWorld;\n'+shader.fragmentShader;
  shader.fragmentShader=shader.fragmentShader.replace('#include <map_fragment>',`#include <map_fragment>
    ...
    float awLeafReadable=1.0-smoothstep(.045,.15,awLeafFootprint);
 ...
 material.customProgramCacheKey=()=>oldKey()+'-forest-leaf-depth-v1';material.needsUpdate=true;
}
...
class GroundContactField{
 mesh:T.InstancedMesh;dummy=new T.Object3D();normal=new T.Vector3();slope=new T.Quaternion();next=0;lastX=Infinity;lastZ=Infinity;
 ...
 update(force=false){
  const player=Object.values(this.landscape.state.players)[0];if(!player)return;const [px,,pz]=player.position,now=performance.now();
  if(!force&&now<this.next&&Math.hypot(px-this.lastX,pz-this.lastZ)<1.1)return;this.next=now+360;this.lastX=px;this.lastZ=pz;
  ...
  (this.landscape as any).__forestContactStats={active:candidates.length,capacity:FOREST_CONTACT_CAPACITY,drawCalls:1};
 }
}
...
function govern(landscape:any){
 ...
 const field=landscape[GRASS_FIELD];
 for(const ring of field?.rings??[]){const id=ring.spec?.id as ObserverRingId|undefined;if(!id)continue;const material=ring.mesh?.material as T.Material|undefined,uniform=material?.userData?.awObserverQuality;if(uniform)uniform.value=observerRingQuality(state.quality,id);}
 landscape.__perceptualBudget={quality:state.quality,smoothedFrameMs:state.controller.smoothedFrameMs,targetMs:state.controller.targetMs};
}
...
function install(){
 ...
 proto.update=function(...args:any[]){const out=oldUpdate.apply(this,args);this[CONTACT]?.update();govern(this);return out;};
}
