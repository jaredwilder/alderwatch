...
// The ranges are intentionally gameplay-first rather than zoological birth rates. A stable slot is
// an ecological opportunity, not a literal immortal individual. Scarcity pressure can accelerate
// these times, while apex species remain deliberately slower to replace.
...
const spawnById=new Map(WILDLIFE_SPAWNS.map(spawn=>[spawn.id,spawn] as const));
...
function alive(a:AnimalState|undefined){return !!a&&!a.dead&&(a.health??species(a.kind).maxHealth)>0;}
function distance2d(a:readonly number[],x:number,z:number){return Math.hypot(a[0]-x,a[2]-z);}
function nearestPlayerDistance(world:WorldState,x:number,z:number){let best=Infinity;for(const p of Object.values(world.players))if(p.health>0)best=Math.min(best,distance2d(p.position,x,z));return best;}
...
export function wildlifeScarcity(world:WorldState,animal:AnimalState){
 ...
 const desired=DESIRED_OCCUPANCY[animal.kind];
 return Math.max(0,1-Math.min(1,speciesRatio/desired,localRatio/.9));
}

export function wildlifeRespawnDelayTicks(world:WorldState,animal:AnimalState){
 const [lo,hi]=RESPAWN_SECONDS[animal.kind],base=lo+(hi-lo)*unitHash(world,animal,'respawn');
 const scarcity=wildlifeScarcity(world,animal);
 const spawn=spawnById.get(animal.id),playerDistance=spawn?nearestPlayerDistance(world,spawn.x,spawn.z):Infinity;
 // The director quietly favors the outer encounter shell around an active player. It never spawns
 // inside the exclusion bubble, but depleted places the player is actually exploring recover first.
 const encounterBias=playerDistance>=WILDLIFE_ENCOUNTER_RING_MIN&&playerDistance<=WILDLIFE_ENCOUNTER_RING_MAX?.72:1;
 const scarcityBias=1-.56*scarcity;
 return Math.max(20*TICKS_PER_SECOND,Math.round(base*encounterBias*scarcityBias*TICKS_PER_SECOND));
}

function clearTransientState(animal:AnimalState){
 animal.dead=false;animal.killedBy=undefined;animal.diedAt=undefined;animal.corpseClearedAt=undefined;
 animal.health=species(animal.kind).maxHealth;animal.maxHealth=species(animal.kind).maxHealth;
 ...
 animal.energy=species(animal.kind).aerial?.maxEnergy;animal.airborne=false;animal.carriedPreyId=undefined;animal.carriedById=undefined;animal.carryUntil=undefined;
 // A respawned slot represents a new wild individual. Criminal history and one-off bounty state
 // belong to the previous animal, not the habitat slot.
...
function respawnPoint(world:WorldState,animal:AnimalState,spawn:WildlifeSpawn,env:WildlifePopulationEnvironment){
 const generation=(animal.spawnGeneration??0)+1,maxJitter=Math.min(14,Math.max(5,species(animal.kind).homeRadius*.48));
 ...
 if(nearestPlayerDistance(world,spawn.x,spawn.z)>=WILDLIFE_PLAYER_EXCLUSION_RADIUS&&!env.blocked?.(spawn.x,spawn.z))return [spawn.x,env.groundY?.(spawn.x,spawn.z)??0,spawn.z] as [number,number,number];
 return undefined;
}
...
export function stepWildlifePopulation(world:WorldState,env:WildlifePopulationEnvironment={}):WildlifePopulationStep{
 ...
 for(const animal of Object.values(world.animals)){
  ...
  if(animal.corpseClearedAt===undefined||corpseAge<wildlifeRespawnDelayTicks(world,animal))continue;
  ...
  const scarcity=wildlifeScarcity(world,animal),playerDistance=nearestPlayerDistance(world,spawn.x,spawn.z),encounter=playerDistance<=WILDLIFE_ENCOUNTER_RING_MAX?1:0;
  const overdue=corpseAge/Math.max(1,wildlifeRespawnDelayTicks(world,animal));
  candidates.push({animal,spawn,score:overdue+scarcity*2.4+encounter*.8+unitHash(world,animal,'queue')*.05});
 }
