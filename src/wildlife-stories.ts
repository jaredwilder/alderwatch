...
function payBeastBounty(w:WorldState,a:AnimalState,t:Tracker){if(!a.bountyClaimed||t.paid.has(a.id))return;const p=Object.values(w.players)[0];if(!p?.bounties?.completedAnimals?.includes(a.id))return;t.paid.add(a.id);const r=ensureRenown(p),reward=animalBountyCrowns(a),before=r.gold;for(const unlocked of recordRenownEvent(p,'complete_bounty',1,w.tick))if(typeof document!=='undefined')achievementPop(unlocked);r.gold+=Math.max(0,reward-(r.gold-before));wildStory('WANTED BEAST: PAID',`${animalDisplayName(a)} is off the board. Mara authorizes ${reward} crowns and absolutely no questions.`);}
