...
export function prepareNpcMaterials(root:T.Object3D,color:string,look?:NpcLook,textures?:Record<string,T.Texture>){const tint=new T.Color(color);root.traverse(o=>{if(/Hood/i.test(o.name))o.visible=look?.hood??o.visible;if(/Hair_Simple/i.test(o.name)&&look)o.visible=!look.hood;if(!(o instanceof T.Mesh))return;o.visible=o.visible!==false;o.castShadow=o.receiveShadow=true;humanoidVisualBounds(o);if(Array.isArray(o.material))o.material=o.material.map(mat=>tintMaterial(mat,tint,look));else o.material=tintMaterial(o.material,tint,look);if(textures)for(const m of (Array.isArray(o.material)?o.material:[o.material]))weatheredCloth(m as T.MeshStandardMaterial,textures,o.name);});return root;}

export class NpcPopulation {
 root=new T.Group();runtimes:NpcRuntime[]=[];private lastTime=0;
 constructor(private scene:T.Scene|T.Group,private assets:Assets,public readonly state:WorldState){this.root.name='Alderbrook townsfolk · adult survivor rig';scene.add(this.root);const survivorIdle=this.assets.survivor.animations.find(a=>/idle/i.test(a.name)),survivorWalk=this.assets.survivor.animations.find(a=>/walk/i.test(a.name));for(const def of NPC_ROSTER){const human=prepareNpcMaterials(assets.human(),def.tint,def.look,assets.textures);const group=new T.Group();group.name=`NPC ${def.name} · ${def.role}`;group.add(human);const tag=label(def);if(tag)group.add(tag);group.position.set(def.x,height(def.x,def.z)+.02,def.z);group.scale.setScalar(.94+(def.phase%3)*.018);this.root.add(group);const mixer=new T.AnimationMixer(human),idle=survivorIdle?mixer.clipAction(survivorIdle):undefined,walk=survivorWalk?mixer.clipAction(survivorWalk):undefined;(walk??idle)?.play();this.runtimes.push({definition:def,group,visual:human,mixer,idle,walk,lastX:def.x,lastZ:def.z});}}
