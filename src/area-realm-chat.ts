...
function install(){
 ...
 const log=panel.querySelector<HTMLElement>('.player-realm-log')!,form=panel.querySelector<HTMLFormElement>('form')!,input=panel.querySelector<HTMLInputElement>('input')!,send=panel.querySelector<HTMLButtonElement>('form button')!,collapse=panel.querySelector<HTMLButtonElement>('.player-chat-collapse')!;
 ...
 form.onsubmit=e=>{e.preventDefault();const text=input.value.trim();if(!text)return;input.value='';append({speakerId:player.id,name:player.name,text:text.slice(0,400),at:Date.now(),human:true});focus();const def=botByMessage(text,messages.length);void (async()=>{const reply=await modelReply(def,text)??fallback(def,text);window.setTimeout(()=>append({speakerId:`player-bot-${def.id}`,name:def.name,text:reply,at:Date.now()}),520+hash(text+def.id)%900);})();};
 // Prevent the Send click from blurring the input first; otherwise pointer capture can
 // reacquire between mousedown and click and swallow the submit in streamed areas.
