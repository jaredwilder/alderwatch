...
const PROFILE:Record<AnimalKind,{flee:number;home:number;wander:number;escape:number}>={
 ...
 deer:{flee:12,home:22,wander:.82,escape:5.6},bear:{flee:4,home:38,wander:.72,escape:1.8},
};
...
export class Nature {
 ...
 update(dt:number){
  ...
  for(const a of Object.values(all)){
   ...
   const prey=a.kind==='bear'?this.bearPrey(a,all):undefined,center=herdCenter(a,all);
   const near=p?dist(p.position,a.position):Infinity,flee=near<profile.flee||!!predator,hunting=!!prey;
   ...
   if(a.kind==='bear'&&prey&&dist(a.position,prey.position)<1.75&&this.w.tick>=(a.attackAt??0)){a.attackAt=this.w.tick+72;bearBite(this.w,a,prey);if(!animalAlive(prey)){a.huntTargetId=undefined;a.huntBestDistance=undefined;a.huntCooldownUntil=this.w.tick+240;}}
