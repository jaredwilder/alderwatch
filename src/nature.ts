...
import {animalAlive,bearBite,ensureAnimalVitals} from './wildlife-rules';

export type AnimalKind='hare'|'crow'|ExtendedAnimalKind;
export interface AnimalState {id:string;kind:AnimalKind;position:Vec3;home:Vec3;yaw:number;phase:number;avoidUntil?:number;health?:number;maxHealth?:number;dead?:boolean;killedBy?:'player'|'bear';diedAt?:number;attackAt?:number}
...
const PROFILE:Record<AnimalKind,{flee:number;home:number;wander:number;escape:number}>={
 ...
 goat:{flee:7,home:14,wander:.58,escape:3.4},sheep:{flee:8,home:14,wander:.48,escape:3.2},
 deer:{flee:11,home:24,wander:.72,escape:5.2},bear:{flee:4,home:36,wander:.55,escape:1.8},
};
const dist=(a:Vec3,b:Vec3)=>Math.hypot(a[0]-b[0],a[2]-b[2]);
export function bearTarget(bear:AnimalState,animals:Record<string,AnimalState>){return Object.values(animals).filter(a=>a.id!==bear.id&&PREY.has(a.kind)&&animalAlive(a)&&dist(a.position,bear.position)<24).sort((a,b)=>dist(a.position,bear.position)-dist(b.position,bear.position))[0];}
...
export function seedNature(w:WorldState){
 ...
 const roster:[string,AnimalKind,number,number,number][]=[
  ...
  ['pasture-goat-0','goat',15,21,.4],['pasture-goat-1','goat',20,27,1.2],['pasture-goat-2','goat',25,34,2.1],['pasture-goat-3','goat',10,39,4.8],
  ['pasture-sheep-0','sheep',-15,20,2.2],['pasture-sheep-1','sheep',-21,26,3.1],['pasture-sheep-2','sheep',-27,34,.7],['pasture-sheep-3','sheep',-12,42,5.4],
  ['southwood-deer-0','deer',28,132,1.7],['southwood-deer-1','deer',-34,178,4.2],['southwood-deer-2','deer',44,164,.9],['southwood-deer-3','deer',-55,118,3.5],['southwood-deer-4','deer',71,206,2.7],['southwood-deer-5','deer',-76,221,5.7],
  ['ironward-bear','bear',198,72,2.8],['briar-bear','bear',-214,154,5.1],['southwood-bear','bear',58,188,3.6],
 ];
...
export class Nature {
 ...
 update(dt:number){
  ...
  for(const a of Object.values(all)){
   ...
   const predator=PREY.has(a.kind)?Object.values(all).filter(b=>b.kind==='bear'&&animalAlive(b)&&dist(b.position,a.position)<16).sort((x,y)=>dist(x.position,a.position)-dist(y.position,a.position))[0]:undefined;
   const prey=a.kind==='bear'?bearTarget(a,all):undefined;
   const near=p?Math.hypot(p.position[0]-a.position[0],p.position[2]-a.position[2]):Infinity,flee=near<profile.flee||!!predator,hunting=!!prey;
   a.phase+=dt;const homeDistance=Math.hypot(a.home[0]-a.position[0],a.home[2]-a.position[2]);
   let desired=predator?Math.atan2(a.position[0]-predator.position[0],a.position[2]-predator.position[2]):flee&&p&&near<profile.flee?Math.atan2(a.position[0]-p.position[0],a.position[2]-p.position[2]):hunting?Math.atan2(prey!.position[0]-a.position[0],prey!.position[2]-a.position[2]):homeDistance>profile.home?Math.atan2(a.home[0]-a.position[0],a.home[2]-a.position[2]):Math.sin(a.phase*.17)*Math.PI;
   const moving=flee||hunting||homeDistance>profile.home||Math.sin(a.phase*.7)>.5,flight=a.kind==='crow'&&moving;
   if((a.avoidUntil??0)>this.w.tick)desired=a.yaw;a.yaw+=T.MathUtils.clamp(T.MathUtils.euclideanModulo(desired-a.yaw+Math.PI,Math.PI*2)-Math.PI,-dt*(a.kind==='bear'?2.8:5),dt*(a.kind==='bear'?2.8:5));
   const speed=hunting?3.9:moving?(flee?profile.escape:flight?2.2:profile.wander):0,x=a.position[0]+Math.sin(a.yaw)*speed*dt,z=a.position[2]+Math.cos(a.yaw)*speed*dt;
   ...
   if(a.kind==='bear'&&prey&&dist(a.position,prey.position)<1.75&&this.w.tick>=(a.attackAt??0)){a.attackAt=this.w.tick+72;bearBite(this.w,a,prey);}
