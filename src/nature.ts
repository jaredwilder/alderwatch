...
export function bearTarget(bear:AnimalState,animals:Record<string,AnimalState>){return Object.values(animals).filter(a=>a.id!==bear.id&&PREY.has(a.kind)&&dist(a.position,bear.position)<24).sort((a,b)=>dist(a.position,bear.position)-dist(b.position,bear.position))[0];}
...
export class Nature {
 ...
 update(dt:number){
  ...
  for(const a of Object.values(all)){
   ...
   const predator=PREY.has(a.kind)?Object.values(all).filter(b=>b.kind==='bear'&&dist(b.position,a.position)<16).sort((x,y)=>dist(x.position,a.position)-dist(y.position,a.position))[0]:undefined;
