...
export class Nature {
 ...
 private async loadAuthoredAnimals(){
  try{
   ...
   for(const a of Object.values(this.w.animals!))if(AUTHORED_ANIMAL_SET.has(a.kind)){
    const kind=a.kind as AuthoredAnimalKind,gltf=library[kind];if(!gltf)continue;
 ...
 update(dt:number){
  ...
  for(const a of Object.values(all)){
   ...
   if(visual.authored){const attacking=(a.attackingUntil??0)>this.w.tick;this.useAction(visual,attacking?visual.attack??(flight?visual.fly:visual.run)??visual.walk:flight?visual.fly??visual.run??visual.walk:moving?((flee||hunting)?visual.run??visual.walk:visual.walk??visual.run):visual.idle??visual.walk);}
